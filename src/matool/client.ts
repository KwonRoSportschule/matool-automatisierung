...
function prepareSchuelerSafeAreaRows(
  ...
  const acceptedIdentifierRows = new Set<SafeAreaRowCapture>();
  ...
  for (const [index, row] of rows.entries()) {
    ...
    acceptedIdentifierRows.add(identifierRow);
  ...
  // Eine sichtbare Zeile ohne Kennung darf fehlen (siehe oben). Eine Kennung
  // oder Aufklapp-Aktion ausserhalb eines zugeordneten Paares dagegen nicht:
  // Dahinter steht ein Mitglied, das sonst still aus der Liste fiele -- und
  // weil die Liste den Bestand exakt ersetzt, auch aus der Datenbank. Nur in
  // den Details eines bereits zugeordneten Mitglieds sind weitere Kennungen
  // Teil dieses Mitglieds und kein eigener Listeneintrag.
  for (const row of rows) {
    if (
      !acceptedIdentifierRows.has(row) &&
      carriesSchuelerIdentity(row) &&
      !isNestedUnderAcceptedIdentifierRow(row, acceptedIdentifierRows)
    ) {
      throw paginatedSafeAreaSchemaError();
    }
  }
...
/** Zeile mit Mitglieds-Kennung oder (auch ungueltiger) Aufklapp-Aktion. */
function carriesSchuelerIdentity(row: SafeAreaRowCapture): boolean {
  return (
...
function isNestedUnderAcceptedIdentifierRow(
  row: SafeAreaRowCapture,
  acceptedIdentifierRows: ReadonlySet<SafeAreaRowCapture>
): boolean {
  let parent = row.parentRow;
  while (parent) {
    if (acceptedIdentifierRows.has(parent)) {
      return true;
    }
